...
class DeclineVerificationRequestTest extends BaseTest
{
    use RefreshDatabaseTrait;

    public function test_That_Access_Denied_Is_Returned_If_Non_Admin_Try_To_Decline_Verification_Request()
    {
    ...
    public function test_That_Success_Response_Is_Returned_And_Status_Updated_If_Admin_Try_To_Approve_Verification_Request()
    {
        ...
        $response = $this->client->request('POST', '/verification_requests/'.$verificationRequestId.'/decline',
            [
                'headers' => [
                    'Authorization' => "Bearer $bearerToken",
                ],
                'json' => ['rejection_message' => 'No reason..Hahaha..']
            ]
        );

        $verificationRequest = $this->getVerifiedRequestById($verificationRequestId);
        $this->assertEquals(EnumVerificationRequestStatusType::TYPE_DECLINED, $verificationRequest->getStatus());
        ...
        $this->assertSame(LocasticMailer::DECLINED_VERIFICATION_SUBJECT, $message->getSubject());
        ...
        $this->assertNotContains(User::ROLE_BLOGGER, $userRoles);
